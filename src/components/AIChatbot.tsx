...
const AIChatbot = ({ lessonTitle, lessonContent }: AIChatbotProps) => {
  ...
  const handleSendMessage = async (e: FormEvent) => {
    ...
    // Format query text to include systemic context & question
    const formattedPrompt = `You are a concise, friendly, and intelligent AI tutor assisting a student who is learning a lesson titled: "${lessonTitle}".
...
The student's question is: ${inputText}`;
    ...
    try {
      ...
      const response = await authFetch(`${backendUrl}/api/gemini-chat`, {
        method: "POST",
        body: JSON.stringify({
      ...
      const responseText = resData.candidates?.[0]?.content?.parts?.[0]?.text || "Sorry, I couldn't process that.";
